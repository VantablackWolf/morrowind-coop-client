...
namespace MWLua
{
    ...
    void LuaManager::jailTimeServed(
        const MWWorld::Ptr& actor, int days, bool ignoreSkillIncreases, std::string_view endText)
    {
        mEngineEvents.addToQueue(
            EngineEvents::OnJailTimeServed{ getId(actor), days, ignoreSkillIncreases, std::string(endText) });
    }
